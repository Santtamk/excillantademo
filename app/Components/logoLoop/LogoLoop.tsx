...
const isNodeLogo = (x: LogoItem): x is NodeLogo => "node" in x;
const isImgLogo = (x: LogoItem): x is ImgLogo => "src" in x;
...
const useResizeObserver = (
  ...
  useEffect(() => {
    // Fallback for browsers/environments without ResizeObserver (SSR safe)
    const RO = (typeof window !== "undefined" ? ResizeObserver : undefined) as
...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    // Snap offset into range when width is known
    if (seqWidth > 0) {
      offsetRef.current =
        ((offsetRef.current % seqWidth) + seqWidth) % seqWidth;
      track.style.transform = `translate3d(${-offsetRef.current}px, 0, 0)`;
